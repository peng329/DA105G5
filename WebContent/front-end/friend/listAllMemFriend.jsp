...
<table>
	...
	<c:forEach var="memNo" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
				
		<tr>

			
			<td>${memSvc.getOneMem(memNo).mem_no}</td>
			<td>${memSvc.getOneMem(memNo).mem_id}</td>
			<td>${memSvc.getOneMem(memNo).mem_name}</td>
<%-- 			<td><%= memSvc.getOneMem("M000001").getMem_name()%></td> --%>
