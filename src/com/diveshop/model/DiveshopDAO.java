...
public class DiveshopDAO implements DiveshopDAO_interface {
	...
	private static final String GET_ALL_STMT = "SELECT * FROM DIVESHOP ORDER BY DS_NO";
	...
	private static final String UPDATE = "UPDATE DIVESHOP SET DS_NAME = ?,BRCID = ?, TEL = ?, ADDRESS = ?,DSACCOUNT = ?, DSPAW = ?, DSMAIL = ?, DSINFO = ?,"
			+ "DS_LATLNG = ?,DS_STATE = ?, DS_ASCORE = ?, DS_REP_NO = ? WHERE DS_NO = ?";
	...
	public void insert(DiveshopVO diveshopVO) {
		...
		try {
			...
			pstmt.setString(10, diveshopVO.getDs_state());
			pstmt.setInt(11, diveshopVO.getDs_ascore());
			pstmt.setInt(12, diveshopVO.getDs_rep_no());
	...
	public void update(DiveshopVO diveshopVO) {
		...
		try {
			...
			pstmt.setInt(11, diveshopVO.getDs_ascore());
			pstmt.setInt(12, diveshopVO.getDs_rep_no());
			pstmt.setString(13, diveshopVO.getDs_no());
	...
	public DiveshopVO findByPrimaryKey(String ds_no) {
		...
		try {
			...
			while (rs.next()) {
				...
				diveshopVO.setAddress(rs.getString("address"));
	...
	public List<DiveshopVO> getAll() {
		...
		try {
			...
			while (rs.next()) {
				diveshopVO = new DiveshopVO();
				diveshopVO.setDs_no(rs.getNString("ds_no"));
				diveshopVO.setDs_name(rs.getString("ds_name"));
				diveshopVO.setBrcid(rs.getInt("brcid"));
				diveshopVO.setTel(rs.getString("tel"));
