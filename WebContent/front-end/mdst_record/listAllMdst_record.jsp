...
<%
    ...
    List<Mdst_recordVO> list = mdst_recordSvc.getAll();
    
    //List<Mdst_recordVO> list = ds.mdst_records.getAll();
...
<html>
<head>
<title>所有員工資料 - listAllMdst_record.jsp</title>
...
<table id="table-1">
	<tr><td>
		 <h3>潛店追蹤資料 - listAllMdst_record.jsp</h3>
...
<table>
	...
	<c:forEach var="mdst_recordVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
		<tr>
			<td>${mdst_recordVO.mem_no}</td>
			<td>${mdst_recordVO.ds_no}</td>
			<td>${mdst_recordVO.trac_time}</td>
